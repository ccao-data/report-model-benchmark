...
Below are the results of our tests. ***Please note that the performance statistics presented here are only for cross-model comparison and do not reflect any real model results e.g. they are included only to show that each framework and version generates similar results, given the same data.***
...
gt() %>%
  ...
  rename(`Wall Time (Full&#160;Run)` = `Wall Time (Full Run)`) %>%
